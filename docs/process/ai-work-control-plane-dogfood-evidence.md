...
- The embedded `StrategyDecision` kept `executes=false`, carried `control_plane_focus=state_context_strategy_topology_approval_evidence_memory_recovery`, exposed `recovery_policy.execution_gate_authority=approved_plan_gate`, and now projects both `route_planner_intent.native_first` and `adapter_shared_contract.comparison_mode=same_contract_two_executors`.
