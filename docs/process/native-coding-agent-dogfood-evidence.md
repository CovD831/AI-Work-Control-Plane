...
This document records the native coding-agent dogfood baseline for a `bounded_internal_repo_task` and keeps the runtime, workspace, UI, CLI, evidence trend reports, and docs on the same productization evidence vocabulary.
...
- governed approval-boundary continuity stays visible across runtime payload, workspace index, UI summary, CLI summary, and evidence reports
